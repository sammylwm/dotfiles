-- DMS user keybind overrides (edit via Control Center or dms; do not remove this header)

hl.unbind("KP_Add")
hl.bind("KP_Add", hl.dsp.exec_cmd("dms ipc call mpris playPause"), { locked = true, description = "dms ipc call mpris playPause" })
hl.unbind("KP_Multiply")
hl.bind("KP_Multiply", hl.dsp.exec_cmd("dms ipc call mpris previous"), { locked = true, description = "dms ipc call mpris previous" })
hl.unbind("KP_Subtract")
hl.bind("KP_Subtract", hl.dsp.exec_cmd("dms ipc call mpris next"), { locked = true, description = "dms ipc call mpris next" })
hl.unbind("SUPER + O")
hl.bind("SUPER + O", hl.dsp.exec_cmd("dms ipc call dankdash wallpaper"), { description = "Wallpaper Browser" })
hl.unbind("SUPER + R")
hl.bind("SUPER + R", hl.dsp.exec_cmd("dms ipc call spotlight toggle"), { description = "dms ipc call spotlight toggle" })
hl.unbind("SUPER + D")
hl.bind("SUPER + D", hl.dsp.workspace.toggle_special("tg"))
hl.unbind("SUPER + L")
hl.bind("SUPER + L", hl.dsp.workspace.toggle_special("localsend"))
hl.unbind("SUPER + M")
hl.bind("SUPER + M", hl.dsp.workspace.toggle_special("music"))
hl.unbind("SUPER + S")
hl.bind("SUPER + S", hl.dsp.workspace.toggle_special("happ"))
hl.unbind("SUPER + SHIFT + right")
hl.bind("SUPER + SHIFT + right", hl.dsp.window.move({ direction = "r" }))
hl.unbind("SUPER + SHIFT + up")
hl.bind("SUPER + SHIFT + up", hl.dsp.window.move({ direction = "u" }))
hl.unbind("SUPER + down")
hl.bind("SUPER + down", hl.dsp.focus({ direction = "d" }))
hl.unbind("SUPER + equal")
hl.bind("SUPER + equal", hl.dsp.window.resize({ x = 100, y = 0, relative = true }), { repeating = true })
hl.unbind("SUPER + left")
hl.bind("SUPER + left", hl.dsp.focus({ direction = "l" }))
hl.unbind("SUPER + minus")
hl.bind("SUPER + minus", hl.dsp.window.resize({ x = -100, y = 0, relative = true }), { repeating = true })
hl.unbind("SUPER + mouse:272")
hl.bind("SUPER + mouse:272", hl.dsp.window.drag(), { description = "Move window" })
hl.unbind("SUPER + mouse:273")
hl.bind("SUPER + mouse:273", hl.dsp.window.resize(), { description = "Resize window" })
hl.unbind("SUPER + right")
hl.bind("SUPER + right", hl.dsp.focus({ direction = "r" }))
hl.unbind("SUPER + up")
hl.bind("SUPER + up", hl.dsp.focus({ direction = "u" }))
hl.unbind("Print")
hl.bind("Print", hl.dsp.exec_cmd("hyprshot -m region"))
hl.unbind("SUPER + B")
hl.bind("SUPER + B", hl.dsp.exec_cmd("zen-browser"))
hl.unbind("SUPER + E")
hl.bind("SUPER + E", hl.dsp.exec_cmd("ghostty -e yazi"), { description = "ghostty -e yazi" })
hl.unbind("SUPER + Return")
hl.bind("SUPER + Return", hl.dsp.exec_cmd("ghostty"), { description = "ghostty" })
hl.unbind("SUPER + T")
hl.bind("SUPER + T", hl.dsp.exec_cmd("ghostty"), { description = "ghostty" })
hl.unbind("SUPER + SHIFT + P")
hl.bind("SUPER + SHIFT + P", hl.dsp.dpms({ action = "toggle" }))
hl.unbind("ALT + CTRL + SHIFT + L")
hl.unbind("ALT + space")
hl.unbind("CTRL + ALT")
hl.unbind("CTRL + space")
hl.unbind("SUPER + CTRL + F")
hl.bind("SUPER + CTRL + F", function() hl.exec_cmd("hyprctl dispatch fullscreenstate") end, { description = "Set Fullscreen State" })
hl.unbind("SUPER + F")
hl.bind("SUPER + F", hl.dsp.window.fullscreen({ mode = "fullscreen", action = "toggle" }), { description = "Toggle maximization" })
hl.unbind("SUPER + Q")
hl.bind("SUPER + Q", hl.dsp.window.close())
hl.unbind("SUPER + SHIFT + E")
hl.unbind("SUPER + SHIFT + F")
hl.bind("SUPER + SHIFT + F", hl.dsp.window.fullscreen({ mode = "maximized", action = "toggle" }), { description = "Toggle fullscreen" })
hl.unbind("SUPER + SHIFT + T")
hl.bind("SUPER + SHIFT + T", hl.dsp.window.float({ action = "toggle" }))
hl.unbind("SUPER + W")
hl.bind("SUPER + W", hl.dsp.group.toggle())
hl.unbind("SUPER + bracketleft")
hl.bind("SUPER + bracketleft", hl.dsp.layout("preselect l"))
hl.unbind("SUPER + bracketright")
hl.bind("SUPER + bracketright", hl.dsp.layout("preselect r"))
hl.unbind("SUPER + code:20")
hl.unbind("SUPER + code:21")
hl.unbind("XF86AudioPlay")
